...
class Subsampler(nn.Module):
    ...
    def forward(self, x, attention_mask, num_tokens=None):
        ...
        # # 找出 attention_mask 为 1 的位置
        # # 例如：mask = [0,0,1,1,1,0,1] -> indices = [2,3,4,6]
        # valid_indices = attention_mask[0].nonzero(as_tuple=False).squeeze(-1)

        # Current call sites pass one utterance at a time. Trim padded frames
        # before temporal subsampling to keep lengths consistent.
        valid_frames = int(attention_mask.sum().item())
        x = x[:, :valid_frames, :]
        # valid_lengths = attention_mask.sum(dim=-1)  # Tensor of shape (B,), e.g., tensor([1135, 413, 283])
        # x_valid = [x[i, :valid_lengths[i], :] for i in range(x.size(0))]
        # print(f"Subsampler input shape: {x.shape}, attention_mask shape: {attention_mask.shape}, attention_mask.sum(dim=-1): {attention_mask.sum(dim=-1)}")
        
        # print(f"Subsampler input dtype: {x.dtype}, attention_mask dtype: {attention_mask.dtype}, attention_mask.sum(dim=-1): {attention_mask.sum(dim=-1)}")
        # print(f"Subsampler input shape: {x.shape}, attention_mask shape: {attention_mask.shape}")
